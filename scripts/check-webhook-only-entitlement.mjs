 ...
 *
 * applyStripeWebhook() (src/lib/server/webhooks.ts) is the only function
 ...
 * directly -- EXCEPT the one dedicated Stripe webhook route below, whose
 * entire job is to verify the Stripe-Signature header and then do exactly
 * that. Every other route (in particular a return/redirect page a browser
 * hits after checkout) must never grant entitlement on its own.
 */
...
const EXEMPT = new Set(["src/routes/api/webhooks/stripe.ts"]);
const BAD = /applyStripeWebhook|insert\s+into\s+billing_event/i;
